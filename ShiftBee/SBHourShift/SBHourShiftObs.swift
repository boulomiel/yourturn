...
//        let start = date.advanced(by: advanced)
    ...
    func fetchDateList() {
        ...
        do {
            ...
            if let dateFound = result.first {
                self.shiftStore.setPersons(dateFound.persons.map { SBPerson(name: $0) })
    ...
    func saveCurrentList() {
        ...
        let moc = modelContainer.mainContext
        let shift = Shift(date: date, startDate: startHour, endDate: endHour, data: SBStorage.archiveStringArray(object: usernames))
        do {
            moc.insert(shift)
            try moc.save()
        } catch {
