...
struct SBAddTeamView: View {
    ...
    // - Constructor
    @State var obs: SBAddTeanObs = .init()
    
    var body: some View {
        NavigationStack {
            VStack {
                SBTitleEditView(title: "New team...") { name in
                    teamName = name
                }
                ...
                Form {
                    Section {
                        ForEach(obs.cellObs, id: \.id) { cellOb in
                            SBSelectionNameCell(obs: cellOb)
    ...
    var saveTeamButton: some View {
        Button {
            ...
            moc.insert(Team(name: teamName, team: names.map { .init(name: $0 )}))
            do {
                try moc.save()
