...
struct SBCSVConverter {
    
    func createCSVFile<T: SBCSVConvertible>(from items: [T]) -> URL? {
    ...
    func generateCSV<T: SBCSVConvertible>(from items: [T]) -> String {
        let header = T.csvHeader
        let rows = items.map { $0.csvRow }
        return ([header] + rows).joined(separator: "\n")
    }
    
    func generatePDF(from people: [SBPerson]) -> URL? {
